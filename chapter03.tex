...
\end{align*}
So $\tilde{r}=r-[\tilde{x}^\mathrm{T}(I-QQ^\mathrm{T})y]^2$. In practice, one can use this update rule to find the $\tilde{x}$ that gives the smallest $\tilde{r}$.
\end{sol}

%3.10
\begin{sol}
Already discussed in \thesection.\ref{3.1}, so just drop the variable with the smallest z-score.
\end{sol}